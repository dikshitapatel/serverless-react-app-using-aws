...
const HotelInfo = () => {
    ...
    useEffect(() => {
     const loadServicesData = async () =>{
       const fetchData = await fetch('https://hhqrdfwigd.execute-api.us-east-2.amazonaws.com/Production/services');
   ...
   useEffect(() => {
    const loadAccesibilitiesData = async () =>{
      const fetchData = await fetch('https://hhqrdfwigd.execute-api.us-east-2.amazonaws.com/Production/accesibilities');
      const data = await fetchData.json();
      setAccesibilitiesData(data);
    }
    ...
    return(
        <div className="scene" id="hotelinfo">
                ...
                <section className="checklist" id="accessibility">
                    ...
                    <ul>
                        {
                            accesibilitiesData.map((accesibileData) => 
                                <li>{accesibileData.name}</li>
                        )
                        }
                    </ul>
                </section>
